...
function cleanMetadataString(str) {
  return (str || "")
  .replace(
    /\s*[\(\[](feat|ft|official|remastered|deluxe|live|bonus|version|video|hq|hd|audio)[^\)\]]*[\)\]]/gi,
    ""
    )
    .replace(/ - Single/gi, "")
    .replace(/\s+/g, " ")
...
function isSystemMenuOpen() {
  const headers = Array.from(
    document.querySelectorAll("h1, h2, h3, .header")
...
function normalizeNuclearTrack(rawTrack) {
  ...
  if (
    ...
    artwork.includes("data:image/svg")
  ) {
    artwork = null;
  }
  ...
  async resolveArtwork(normalizedTrack) {
    ...
    try {
      const query = encodeURIComponent(
        `${cleanMetadataString(artist)} ${cleanMetadataString(title)}`
      );

      const itunesRes = await httpFetch(
...
function updateMediaSession(artist, title, artUrl) {
  ...
  try {
    ...
    navigator.mediaSession.setActionHandler(
      "play",
      () => {
        if (storedApi?.Playback?.play) {
          storedApi.Playback.play();
        } else {
    ...
    navigator.mediaSession.setActionHandler(
      "pause",
      () => {
        if (storedApi?.Playback?.pause) {
          storedApi.Playback.pause();
        } else {
          const a = getAudioElement();
          if (a) a.pause();
        }
...
const Fetchers = {
  lrclib: async (artist, title, signal) => {
    try {
      ...
      const res = await httpFetch(
        `https://lrclib.net/api/get?${strict}`,
        { signal }
      );
      ...
      if (res.ok) {
        ...
        const lyrics = AppState.preferences.preferSynced
  ...
  netease: async (artist, title, signal) => {
    try {
      ...
      const searchRes = await httpFetch(
        `https://music.163.com/api/search/get/web?csrf_token=type=1&offset=0&total=true&limit=3&s=${query}`,
        { signal }
      );
      ...
      const lrcRes = await httpFetch(
        `https://music.163.com/api/song/lyric?os=pc&id=${data.result.songs[0].id}&lv=-1&kv=-1&tv=-1`,
        { signal }
      );

      const lyricText =
      (await lrcRes.json())?.lrc?.lyric;

      if (
        lyricText &&
        !lyricText.includes("纯音乐")
      ) {
        return {
          lyrics: lyricText,
          source: "netease",
  ...
  kugou: async (artist, title, signal) => {
    try {
      ...
      const res = await httpFetch(
        `https://lyrics.kugou.com/search?ver=1&man=yes&client=pc&keyword=${query}&duration=&hash=`,
        { signal }
      );
...
async function fetchLyrics(artist, title) {
  ...
  const cA =
  cleanMetadataString(artist);

  const cT =
  cleanMetadataString(title);
...
function cleanLyricsText(rawText) {
  ...
  if (
    !AppState.preferences.cleanLyrics
  ) {
    return rawText;
  }
  ...
  return rawText
  .replace(
    /^\[(Verse|Chorus|Bridge|Outro|Intro|Hook|Pre-Chorus)[^\]]*\]$/gim,
           ""
  )
  .replace(
    /^.*by.*lyrics.*$/gim,
...
function parseLrc(raw) {
  ...
  lines.forEach((line) => {
    ...
    if (
      /^\[(ar|ti|al|au|length|by|offset|re|ve):/i.test(
        trimmed
      )
    ) {
      return;
    ...
    if (timestamps.length > 0) {
      ...
      AppState.parsedLyrics.length >
      0 &&
      !AppState.parsedLyrics[
        AppState.parsedLyrics.length - 1
      ].spacer
    ) {
      AppState.parsedLyrics.push(
        {
  ...
  return AppState.parsedLyrics.sort(
    (a, b) =>
    a.time - b.time
  );
}
...
function injectStyles() {
  ...
  "nuke-mp-styles";
    ...
    .nuke-btn-icon-card:hover {
      opacity: 1;
      transform: scale(1.1);
    }

    .nuke-btn-icon-card.disabled {
    ...
    .nuke-source-badge {
      font-size: 11px;
      color: var(--nuke-mp-accent);
      font-weight: 700;
      text-transform: uppercase;
      letter-spacing: 0.8px;
    }

    .nuke-toolbar-actions {
      display: flex;
      gap: 8px;
      align-items: center;
    }
    ...
    .lyric-line.active {
      color: var(--nuke-mp-accent) !important;
      font-size: 1.15em;
      font-weight: 800;
      transform: scale(1.03);
      background: rgba(255,255,255,0.06);
      text-shadow: 0 0 12px rgba(30,215,96,0.3), 0 1px 4px rgba(0,0,0,0.6);
    }
    ...
    .nuke-queue-item:hover {
      background: rgba(255,255,255,0.06);
    }

    .nuke-queue-item.active {
...
function injectUI() {
  ...
  "nuke-mp-panel";
    ...
    <div class="nuke-switch-bg" id="nuke-sw-glow">
    <div class="nuke-switch-knob"></div>
    </div>
    </div>

    <div class="nuke-setting-row">
    <span>Show Artwork Background</span>
    <div class="nuke-switch-bg" id="nuke-sw-bg">
    <div class="nuke-switch-knob"></div>
    </div>
    </div>
    ...
    <div class="nuke-switch-bg" id="nuke-sw-clean">
    <div class="nuke-switch-knob"></div>
    </div>
    </div>
    ...
    <div class="nuke-switch-bg" id="nuke-sw-queue">
    <div class="nuke-switch-knob"></div>
    </div>
    </div>

    <div class="nuke-setting-row">
    <span>Auto Scroll Lyrics</span>
    <div class="nuke-switch-bg" id="nuke-sw-autoscroll">
    <div class="nuke-switch-knob"></div>
    </div>
    </div>
    ...
    <div class="nuke-switch-bg" id="nuke-sw-synced">
    <div class="nuke-switch-knob"></div>
    </div>
    </div>
    </div>
...
async function updateMiniPlayerMeta(
  normalizedTrack
) {
  const panel =
  document.getElementById(
  ...
  const artUrl =
  await ArtworkCache.resolveArtwork(
    normalizedTrack
  );
  ...
  if (artUrl) {
    AppState.lastTrackArt =
    artUrl;

    artworkEl.style.backgroundImage =
    AppState.preferences.showBgColor
    ? `url(${artUrl})`
    : "none";
...
function updateLyricsUI(
  ...
  if (
    ...
    body.innerHTML =
    `<div style="color: #777; margin-top: 40px; font-weight:600;">no lyrics available</div>`;

    AppState.currentLyricText =
    "No lyrics available";

    return;
  }

  AppState.parsedLyrics.forEach(
    (line) => {
      const lineEl =
      document.createElement(
        "div"
      ...
      `${AppState.preferences.fontSize}px`;
...
function updateSliderBackground(
  ...
  `linear-gradient(to right, var(--nuke-mp-accent) 0%, var(--nuke-mp-accent) ${val}%, rgba(255,255,255,0.2) ${val}%)`;
}
...
async function refreshQueueButtonState(
  force = false
) {
  const now = Date.now();
  ...
  if (
    !force &&
    now - lastQueueCheckAt <
    QUEUE_CHECK_INTERVAL
  ) {
    return;
  }

  if (queueCheckInProgress) {
    return;
  }
 ...
 *
 * Queue playback is preferred because it lets us select
 * the exact target track instead of depending exclusively
 * on version-specific Playback.previousTrack()/nextTrack().
 *
 * Direct Playback methods remain as a fallback.
 */
async function skipTrack(
  ...
  try {
    ...
    if (
      ...
      // Some queue APIs expose upcoming items
      // without the current track. In that case,
      // the first item is the natural "next" item.
      if (currentIndex === -1) {
        if (
          direction === "next" &&
          queue.length > 0
        ) {
          await queueApi.playTrack(
            queue[0]
          );
...
function attachPanelEventListeners(
      ...
      if (!drag.rafId) {
        ...
        requestAnimationFrame(
          () => {
            ...
            `translate3d(${drag.translateX}px, ${drag.translateY}px, 0)`;
  ...
  setupSwitch(
    ...
    .ambientGlow,
    async (v) => {
      AppState.preferences.ambientGlow =
      v;

      await storedApi?.Settings?.set(
        SETTINGS_IDS.AMBIENT_GLOW,
        v
      );
  ...
  setupSwitch(
    ...
    .showQueue,
    async (v) => {
      AppState.preferences.showQueue =
      v;

      await storedApi?.Settings?.set(
        SETTINGS_IDS.SHOW_QUEUE,
        v
      );
  ...
  () => {
    ...
    if (
      queueButton?.classList.contains(
        "disabled"
      )
    ) {
      return;
...
// fallback queue engine
async function getQueueArray() {
  if (!storedApi) return [];

  // 1. Nuclear plugin API
  try {
    if (
      storedApi.Queue
      ?.getQueue
      ...
      const q =
      await storedApi.Queue.getQueue();
    ...
    if (
      ...
      const q =
      await storedApi.Queue.getTracks();
    ...
    if (
      ...
      const q =
      await storedApi.Queue.getItems();
  ...
  try {
    ...
    document.querySelectorAll(
      ".queue-item, [class*='QueueItem'], [class*='queue_item']"
    );
    ...
    if (
      ...
      queueElements.forEach(
        (el) => {
          const titleEl =
          el.querySelector(
            ".title, [class*='title'], h4, span"
          );

          const artistEl =
          el.querySelector(
            ".artist, [class*='artist'], p"
          );
...
async function renderQueueDropdown() {
  ...
  `<div style="padding:16px; color:#888;">fetching queue...</div>`;
  ...
  try {
    ...
    if (
      ...
      listContainer.innerHTML =
      `<div style="padding:16px; color:#888;">no upcoming tracks in queue</div>`;
    ...
    queue.forEach(
      (item, idx) => {
        ...
        li.innerHTML = `
        <span style="font-size:11px; color:#666; width:16px;">
        ${idx + 1}
        </span>

        <div style="flex:1; min-width:0;">
        <div style="font-size:13px; font-weight:600; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">
        ${norm.title || "unknown track"}
        </div>
        ...
        li.addEventListener(
          ...
          async () => {
            if (
              ...
              await storedApi.Queue.playTrack(
                item
              );
            }
          }
...
function setupNativeSettingsInjector(
  api
) {
  if (settingsObserver) {
  ...
  new MutationObserver(
    ...
    "display: flex; align-items: center; gap: 12px; cursor: pointer; margin-top: 6px;";

      wrapper.innerHTML = `
      <div style="width: 40px; height: 22px; background: ${
        isChecked
        ? "var(--nuke-mp-accent, #1ed760)"
        : "#444"
      }; border-radius: 11px; position: relative; transition: 0.2s;">
      <div style="width: 18px; height: 18px; background: #fff; border-radius: 50%; position: absolute; top: 2px; left: ${
        isChecked
        ? "20px"
        : "2px"
      }; transition: 0.2s;"></div>
      ...
      injectToggle(
        "enable ambient background color aura (better lyrics)",
                   SETTINGS_IDS.AMBIENT_GLOW
      );

      injectToggle(
        "clean lyrics section headers and credits",
        SETTINGS_IDS.CLEAN_LYRICS
      );

      injectToggle(
        "display miniplayer background color",
        SETTINGS_IDS.SHOW_BG_COLOR
      );
...
module.exports = {
  ...
  async onEnable(api) {
    ...
    if (
      ...
      unsubscribeQueue =
      api.Queue.subscribeToCurrentItem(
        (item) => {
          if (item?.track) {
            handleTrackChange(
              api,
